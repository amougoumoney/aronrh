...
const columns = [
  ...
  },
  {
    ...
    sorter: {
      compare: (a, b) => {
        a = a.Emp_ID.toLowerCase();
        b = b.Emp_ID.toLowerCase();
...
<template>
  <a-table class="table datatable thead-light" :columns="columns" :data-source="data" :row-selection="rowSelection">
    <template #bodyCell="{ column, record }">
      <template v-if="column.key === 'Emp_ID'">
        <router-link to="/employee/employee-details" class="link-default">{{
          record.Emp_ID
        }}</router-link>
      </template>
      <template v-if="column.key === 'Name'">
        <div class="d-flex align-items-center">
          <a href="#" class="avatar avatar-md" data-bs-toggle="modal" data-bs-target="#view_details"><img
              :src="`@/assets/img/users/${record.Image}`" class="img-fluid rounded-circle" alt="img" /></a>
          <div class="ms-2">
            <p class="text-dark mb-0">
              <a href="#" data-bs-toggle="modal" data-bs-target="#view_details">{{ record.Name }}</a>
            </p>
